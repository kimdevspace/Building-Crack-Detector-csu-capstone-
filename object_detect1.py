...
def atsave_image():# auto save img
    ...
    if im0 is not None:
        ...
        if current_time - last_save_time >= 10:
            label_name = "saved_image_" + str(int(time.time())) + ".jpg"
            cv2.imwrite(save_path + '/' + label_name, im0)
            image1=Image.open(save_path+'/'+label_name,im0)
            if image1.mode == 'RGB':
                image1_bytes=image1.tobytes()
                blob=bucket.blob(label_name)
                blob.upload_from_filename(save_path+'/'+label_name,im0)
...
def run():
   ...
   # model.warmup(imgsz=(1, 3, *imgsz), half=half)  # warmup
    ...
    while(True):
        ...
        # in run -> im0 update and image save event
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if 'crack' in label:
                        ...
                        cv2.putText(im0, str(leng)+'cm',
                            ...
                            0.57,
                            (0, 0, 255),2)
                        # image save###   realtime image save code
                        #if save_img:
                            #label_name = label + '_' + str(round(time.time())) + '.jpg'
                            #cv2.imwrite(save_path + label_name, im0)
...
#save button
#save_button = ttk.Button(root, text="image save", command=save_image)
#save_button.pack()
